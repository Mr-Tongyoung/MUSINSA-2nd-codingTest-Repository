...
### Q15. 프로젝트를 다시 한다면 개선하고 싶은 점은?

**A**: 3가지를 개선하겠다.

**1. 캐싱 도입**

강좌 목록 조회(`GET /courses`)는 읽기 전용이므로 Spring Cache(`@Cacheable`)를 적용하여 DB 부하를 줄일 수 있다. 수강신청/취소 시 해당 강좌의 캐시를 무효화(`@CacheEvict`)하면 정합성도 유지된다.
